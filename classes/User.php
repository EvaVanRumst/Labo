<?php
	class User
	{
		...
		public function exists( $email )
		{
			...
			$userExistsResult = $this->db->query( $userExistsQuery, $userExistsPlaceholders );
		...
		public function create( $email, $password )
		{
			...
			$placeholders 	=	array( ":email" => $email, 
										...
										":salt" => $salt );
			// Query uitvoeren
			$this->db->query( $createQuery, $placeholders );
		}
		...
		public function login( $email, $password )
		{
			...
			if ( $salt !== '' )
			{
				...
				$userDataResult	= $this->db->query( $userDataQuery, $userDataPlaceholders );
...
//die();
		...
		public function getSalt( $email )
		{
			...
			$saltResult	=	$this->db->query( $saltQuery, $saltPlaceholders );
